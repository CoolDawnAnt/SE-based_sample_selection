...
parser.add_argument('--dataset', type=str, default='cifar10',
                    choices=['cifar10', 'cifar100', 'tiny', 'svhn', 'cinic10'])

######################### Path Setting #########################
parser.add_argument('--data-dir', type=str, default='../data/',
                    help='The dir path of the data.')
parser.add_argument('--base-dir', type=str,
                    help='The base dir of this project.', default='./data-model/cifar100')
parser.add_argument('--task-name', type=str, default='all-data2',
...
data_score_path = os.path.join(task_dir, f'data-score-{args.task_name}-base.pickle')
...
def training_dynamics_metrics(td_log, dataset, data_importance):
    ...
    for i in range(data_size):
        _, (_, y) = dataset[i]
        targets.append(y)
    with open(f'./graphs/{args.dataset}-targets.pickle', 'wb') as f:
        pickle.dump(targets, f)
    targets = torch.tensor(targets)
    data_importance['targets'] = targets.type(torch.int32)
    data_importance['entropy'] = torch.zeros(data_size).type(torch.float32)
    ...
    Losses = [0 for i in range(200)]

    def record_training_dynamics(td_log):

        logit = td_log['output'].type(torch.float)
        ...
        data_importance['forgetting'][index] += torch.logical_and(data_importance['last_correctness'][index] == 1,
                                                                  correctness == 0)
        ...
        batch_idx = range(logit.shape[0])
        # target_logit = logit[batch_idx, label]
        # logit[batch_idx, label] = -1e9
        # other_highest_logit = torch.max(logit, dim=1)[0]
        # margin = target_logit - other_highest_logit
        # data_importance['accumulated_margin'][index] += margin

        target_prop = output[batch_idx, label]
        output[batch_idx, label] = -1e9
        other_highest_prop = torch.max(output, dim=1)[0]
        margin = target_prop - other_highest_prop
        data_importance['accumulated_margin'][index] += margin

        entropy = -1 * output * torch.log(output + 1e-10)
        entropy = torch.sum(entropy, dim=1).detach().cpu()
        data_importance['entropy'][index] += entropy
...
def EL2N(td_log, dataset, data_importance, max_epoch=10):
    ...
    def record_training_dynamics(td_log):
        output = F.softmax(td_log['output'].type(torch.float), dim=1)
