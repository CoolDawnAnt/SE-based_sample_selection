...
import copy
def SE_bns(dataset, ratio, mis_ratio, gamma = 1, remain_version=1, mis_r = 0, scores = None, scores2 = None, graph = 'imagenet-train-6NN_se', dfn_version = 0, cut_balance = 0, reverse = True):
    n = len(dataset)
    m = int(n * ratio)
    labels = []
    for i in range(n):
    ...
    print(f'Using SE_bns on Graph {graph} with cut_balance = {cut_balance} mis = {mis_ratio} dfn = {dfn_version} gamma = {gamma} bin = {remain_version}')

    data_dir = './graphs/'
    undirected_adj = sparse.load_npz(f'{data_dir}{graph}.npz')
    ...
    for i in idx:
        sums[labels[i]] += 1
    if (len(idx) < m):
        print(f'Not enough samples! Needs {m} samples, only select {len(idx)} samples (This means imbalance factor gamma needs bigger)')
